...
public interface ScheduleRepository extends PagingAndSortingRepository<Schedule, Integer>{
    ...
    @Query("select new com.edumoca.soma.entities.models.ScheduleResponse(sch.scheduleId,sch.scheduleName,sch.scheduleDateTime,sch.session.period.periodName,sch.session.period.periodNo,sch.startDate,sch.endDate,sch.session.sessionId,sch.session.startTime,sch.session.endTime,sch.recurring,sch.fullDay,sch.teacherGradeSectionSubjectMap.teacher.userId,sch.teacherGradeSectionSubjectMap.teacher.firstName,sch.teacherGradeSectionSubjectMap.subject.subjectId,sch.teacherGradeSectionSubjectMap.subject.subjectName) from Schedule sch where sch.startDate <= :startDate and sch.endDate >= :endDate and sch.session.startTime >= :sessionStartTime and sch.session.endTime <= :sessionEndTime and sch.recurring = true")
    List<ScheduleResponse> getSchedulesByStartDateAndEndDateBetween(@Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("sessionStartTime") LocalTime sessionStartTime,@Param("sessionEndTime") LocalTime sessionEndTime);
